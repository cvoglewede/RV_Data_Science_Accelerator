...
```{r}
missingVec <- c(1,0,8,20,84,2,94,NA,20,NA,1,0,8,20,84,3,20,NA,10,84,20,82,6,94,NA,20,NA)
sum(is.na(missingVec))
length(missingVec)
sum(is.na(missingVec))/length(missingVec)
```
...
ggplot(data = diamonds2) + 
  geom_point(mapping = aes(x = log(carat), y = resid))
```
